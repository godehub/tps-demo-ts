import { ConfigFile, Node, Viewport, Window, RenderingServer, DisplayServer, Engine } from "godot";
...
export default class Settings extends Node {
	...
	DEFAULTS: SettingsDefaults = {
		video: {
			...
			vsync: DisplayServer.VSYNC_ENABLED ?? 1,
			max_fps: 0,
		...
		rendering: {
			...
			ssao_quality: RenderingServer.ENV_SSAO_QUALITY_MEDIUM ?? 2,
			ssil_quality: -1,
			bloom: true,
			volumetric_fog: true } };
	...
	apply_graphics_settings(window: WindowType, environment: Environment, scene_root: NodeType): void {
		...
		DisplayServer.window_set_vsync_mode(this.value("video", "vsync"));
		Engine.max_fps = this.value("video", "max_fps");
		...
		} else if (ssaoQuality === RenderingServer.ENV_SSAO_QUALITY_MEDIUM) {
		...
		} else if (ssilQuality === RenderingServer.ENV_SSIL_QUALITY_MEDIUM) {
